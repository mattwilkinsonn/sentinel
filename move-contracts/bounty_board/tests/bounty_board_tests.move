...
use bounty_board::{
    bounty_board,
    config::{Self, ExtensionConfig, AdminCap},
};
...
fun test_double_claim_fails() {
    ...
    let killmail_id = create_killmail(
        &mut ts, poster_char_id, CHARACTER_B_ITEM_ID as u64, VICTIM_GAME_ID,
    );
    ...
    ts::next_tx(&mut ts, user_a());
    {
        ...
        bounty_board::post_bounty(
            &mut board, &extension_config, &mut storage_unit, &character, &owner_cap,
            VICTIM_GAME_ID, tenant(), REWARD_TYPE_ID, REWARD_QUANTITY, 0, &clock,
            ts::ctx(&mut ts),
        );
        character.return_owner_cap(owner_cap, receipt);
        ts::return_shared(character);
    ...
    ts::next_tx(&mut ts, user_b());
    {
        ...
        let killmail = ts::take_shared_by_id<killmail::Killmail>(&ts, killmail_id);
        bounty_board::claim_bounty(
            &mut board, &extension_config, &mut storage_unit, &hunter, &killmail,
            1, &clock, ts::ctx(&mut ts),
        );
        ts::return_shared(killmail);
...
fun test_cancel_by_non_poster_fails() {
    ...
    ts::next_tx(&mut ts, user_b());
    {
        ...
        let character = ts::take_shared_by_id<Character>(&ts, poster_char_id);
        bounty_board::cancel_bounty(
            &mut board, &mut storage_unit, &character, 1, ts::ctx(&mut ts),
        );
        abort 999
    }
}
