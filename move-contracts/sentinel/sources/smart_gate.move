...
module sentinel::smart_gate {
    use sentinel::{config::{Self, AdminCap, ExtensionConfig}, threat_registry::ThreatRegistry};
    use sui::clock::Clock;
    use world::{character::Character, gate::Gate};
    ...
    public fun request_passage(
        ...
        // Load threshold
        assert!(extension_config.has_rule<GateThresholdKey>(GateThresholdKey {}), EThresholdNotSet);
        let threshold = extension_config.borrow_rule<
            GateThresholdKey,
            GateThreshold,
        >(GateThresholdKey {});
    ...
    public fun get_gate_threshold(config: &ExtensionConfig): u64 {
        assert!(config.has_rule<GateThresholdKey>(GateThresholdKey {}), EThresholdNotSet);
