...
// default branch. Dev/preview stages branch off it for scale-to-zero.
//
// After deploying production for the first time, set the project ID on dev:
//   pulumi config set neonProjectId <id> -s sentinel/dev
//
// If neonProjectId is not set, the stack creates its own standalone project.
...
const neonProjectIdConfig = config.get("neonProjectId");

if (neonProjectIdConfig) {
  // Branch off an existing Neon project (dev/preview stages)
  const parentProjectId = pulumi.output(neonProjectIdConfig);
  ...
  // Create a new Neon project (production, or standalone dev before prod exists)
  const projectName = isProduction ? "sentinel" : `sentinel-${stack}`;
