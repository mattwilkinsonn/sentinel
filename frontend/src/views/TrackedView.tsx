...
export function TrackedView(props: TrackedViewProps) {
  const sorted = () => [...props.profiles].sort((a, b) => b.threat_score - a.threat_score);
  ...
  return (
    <div>
      <h3 class="text-lg tracking-wider" style="margin-bottom:1rem">ALL TRACKED CHARACTERS ({props.profiles.length})</h3>
          ...
          {(profile) => {
            ...
            return (
              ...
              >
                ...
                <span class={`badge ${colorClass()}`} style={{ background: `${color()}15` }}>
                  {tier()}
                </span>
