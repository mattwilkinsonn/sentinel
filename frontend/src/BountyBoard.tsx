...
import { Crosshair, Clock, User, Users, Target, Trophy, RefreshCw } from "lucide-solid";
...
export function BountyBoard() {
  ...
  async function fetchBounties() {
    ...
    try {
      ...
      const result = await client.getDynamicFields({ parentId: BOUNTY_BOARD_ID });
      const loaded: Bounty[] = [];
      ...
      for (const field of result.data) {
        try {
          ...
          const contributors: Contribution[] = (fields.contributors || []).map((c: any) => ({
    ...
    <div>
      ...
      <Show when={!BOUNTY_BOARD_ID}>
        <div class="glass-card p-8 text-center">
          <h3 class="text-lg tracking-wider mb-3 text-accent-cyan">AWAITING DEPLOYMENT</h3>
          <p class="text-text-secondary text-sm mb-4">
            The bounty board contract needs to be deployed and configured before bounties can be displayed.
          </p>
          <div class="text-xs text-text-muted">
            ...
            <code class="block mt-2 p-3 rounded bg-bg-primary border border-border-default text-left">
              just contracts-deploy<br/>
              just script bounty_board/create-board<br/>
              # Then set VITE_BOUNTY_BOARD_ID in frontend/.env
            </code>
          </div>
        </div>
      </Show>
      ...
      <div class="flex items-center justify-between" style="margin-bottom:1.5rem">
        <h3 class="text-lg tracking-wider">
          ACTIVE BOUNTIES <span class="text-text-muted text-sm">({active().length})</span>
        </h3>
        <button
          onClick={() => { fetchBounties(); fetchEvents(); }}
          disabled={loading()}
        ...
        <div class="flex-1 min-w-0">
          ...
          >
            ...
            <Show when={claimed().length > 0}>
              <div class="border-t border-border-default" style="margin-top:2rem;margin-bottom:1.5rem" />
              <h3 class="text-lg tracking-wider text-text-muted" style="margin-bottom:1rem">
        ...
        <Show when={events().length > 0}>
          <div class="w-72 shrink-0 hidden lg:block">
            <div class="glass-card p-4">
              <h4 class="text-sm tracking-wider flex items-center gap-2 text-text-secondary" style="margin-bottom:1rem">
                <div class="live-dot" />
                BOUNTY FEED
              </h4>
              <div class="flex flex-col gap-2">
                <For each={events()}>
                  {(ev) => {
                    const config: Record<string, { icon: typeof Crosshair; color: string; verb: string }> = {
                      posted: { icon: Crosshair, color: "text-accent-cyan", verb: "posted" },
                      claimed: { icon: Trophy, color: "text-accent-green", verb: "claimed" },
                      cancelled: { icon: Target, color: "text-accent-red", verb: "cancelled" },
                      stacked: { icon: Users, color: "text-accent-blue", verb: "stacked on" },
                    };
                    ...
                    return (
                      <div class="flex items-start gap-2 text-xs py-1.5" style={`border-left:2px solid var(--color-accent-cyan)`}>
                        <c.icon size={12} class={`shrink-0 mt-0.5 ml-2 ${c.color}`} />
                        <div class="min-w-0">
                          <span class="text-text-secondary">
                            {abbreviate(ev.actor)} <span class={c.color}>{c.verb}</span> bounty #{ev.bountyId}
                          </span>
...
function BountyCard(props: { bounty: Bounty }) {
  ...
  const isUrgent = !b.claimed && !isExpired() && expiresIn < 7_200_000 && expiresIn > 0;
      ...
      <div class="flex justify-between items-start gap-4">
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-2 mb-2 flex-wrap">
            <span class="font-bold text-text-primary tracking-wide">BOUNTY #{b.id}</span>
            {b.claimed && <span class="badge bg-accent-green/15 text-accent-green">CLAIMED</span>}
            {isExpired() && <span class="badge bg-accent-red/15 text-accent-red">EXPIRED</span>}
            {hasMultiple && (
        ...
        <div class="text-right shrink-0">
          <div class={`text-2xl font-bold ${tier.class}`}>{b.reward_quantity}x</div>
          <div class="text-xs text-text-muted">Type #{b.reward_type_id}</div>
          <div class={`text-xs mt-0.5 ${tier.class}`}>{tier.label}</div>
          <Show when={!b.claimed}>
            <div class={`flex items-center justify-end gap-1 mt-2 text-xs ${
