...
export async function hydrateWorldConfig(
  ...
  if (!hasManualIds) {
    ...
    if (
      !extracted?.world ||
      extracted.world.packageId !== ctx.config.packageId
    ) {
      const filePath = getExtractedObjectIdsPath(network);
      throw new Error(
        `Missing or mismatched ${filePath}. Deploy world-contracts first.`,
      );
    }
...
export function loadExtractedObjectIds(
  network: string,
): ExtractedObjectIds | null {
  const filePath = getExtractedObjectIdsPath(network);
...
export function getDefaultWorldPackageId(network: string): string {
  return (
    ...
    loadExtractedObjectIds(network)?.world?.packageId ||
    ""
  );
}
